...
tvd_fmt_t       tvd_format      = TVD_PL_YUV420;
...
void capture_yuv420_raw_data(int width,int height,void *y_addr,void *c_addr)
{
        ...
        fwrite(c_addr,1,width*height/2,c_file);
        fclose(c_file);
}
int disp_init(int w,int h,layerinitpara_t *layer_info)
{
	...
	if((disphd = open("/dev/disp",O_RDWR)) == -1)
	{
        ...
        if (layer_info->h < 720)
        {
		//Q:这里的这个CS_MODE具体表示些什么意思？
		// 为何有以layer_info->h 做为DISP_BT601和DISP_BT709之分？
        ...
        layer_para.src_win.width      = layer_info->w;
        layer_para.src_win.height     = layer_info->h;
...
static int read_frame (void)
{
    ...
    if(first_frame)
    { 
        capture_yuv420_raw_data(layerinit.w,layerinit.h,(void*)buffers[buf.index].start,(void*)buffers[buf.index].start+layerinit.w*layerinit.h);
    }
...
int get_system(tvd_system_t *system)
{
	int ret = 0;	
    ...
    while(i < argc)
	{
		...
		if(!strcmp(argv[i], "-i")){
			if (argc > i+1) {	
				...
				if(!strcmp(argv[i], "0"))
                                {                            
                                        layerinit.format        = HWC_FORMAT_DEFAULT;
                                        tvd_format              = TVD_PL_YUV420; 
                                }
                                else if(!strcmp(argv[i], "1"))
                                {
				...
				if(!strcmp(argv[i], "1"))
                                {                            
					tvd_channel  = TVD_CHANNEL_ONLY_1;
                                }
                                else if(!strcmp(argv[i], "2"))
                                {
					tvd_channel  = TVD_CHANNEL_ONLY_2;
                                ...
                                {
					tvd_channel  = TVD_CHANNEL_ONLY_3;
                                }
                                else if(!strcmp(argv[i], "4"))
                                {
					tvd_channel  = TVD_CHANNEL_ONLY_4;
                                }
                ...
                if(!strcmp(argv[i], "-h")){
                        printf("usage:\n"
                             "\t-i interface    0 = cvbs       1 = ypbpr\n"
                             "\t-f fomat        0 = PL_YUV420  1 = MB_YUV420\n");
                        return -1;
                }
                i++;
	}
