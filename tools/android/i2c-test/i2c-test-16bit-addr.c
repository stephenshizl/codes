...
static int i2c_writes(int id, int addr, u16 *buf, int bytes)
{
	...
	data[0] = (u8) (buf[0] >> 8);
	data[1] = (u8) (buf[0] & 0xff);
	
	data[2] = (u8) (buf[1] & 0xff);
...
static int i2c_reads(int id, int addr, u16 *buf, int bytes)
{
	struct i2c_adapter *adap;
		...
		const char *buf, size_t size)
{
	...
	while (count < size) {
		(*poiter) = (u16)simple_strtol(buf, &after, 16);
		if (after == buf)
			return -EINVAL;
		poiter++;
		count += after - buf;
