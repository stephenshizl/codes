...
static int tps65910_chip_init(struct tps65910_reg *pmic)
{
	u8 value;

	/* XXX PWM mode */
	value = tps65910_reg_read(pmic, TPS65910_DCDCCTRL);
	value &= ~((1<<3)|(1<<4)|(1<<5));
	tps65910_reg_write(pmic, TPS65910_DCDCCTRL, value);

	/* XXX Give control of all register to control port */
	value = tps65910_reg_read(pmic, TPS65910_DEVCTRL);
	value |= 1<<4;
	tps65910_reg_write(pmic, TPS65910_DEVCTRL, value);

	/* XXX VDD2:1.5A/2.5mV/us VIO:1A/3.3V */
	tps65910_reg_write(pmic, TPS65910_VDD2, 0x3d);
	tps65910_reg_write(pmic, TPS65910_VIO, 0x8d);

	/*for VDD2&VIO auto enter pfm mode when sleep*/
	value = tps65910_reg_read(pmic, TPS65910_EN1_SMPS_ASS);
	value &= ~((1<<2)|(1<<0));
	tps65910_reg_write(pmic, TPS65910_EN1_SMPS_ASS, value);

	value = tps65910_reg_read(pmic, TPS65910_EN2_SMPS_ASS);
	value &= ~((1<<2)|(1<<0));
	tps65910_reg_write(pmic, TPS65910_EN2_SMPS_ASS, value);

	value = tps65910_reg_read(pmic, TPS65910_SLEEP_KEEP_RES_ON);
	value &= ~((1<<2)|(1<<0));
	tps65910_reg_write(pmic, TPS65910_SLEEP_KEEP_RES_ON, value);

	/* SLEEPSIG_POL: sleep active signal low */
	value = tps65910_reg_read(pmic, TPS65910_DEVCTRL2);
	value &= ~(1<<3);
	tps65910_reg_write(pmic, TPS65910_DEVCTRL2, value);

	/* allow sleep device state */
	value = tps65910_reg_read(pmic, TPS65910_DEVCTRL);
	value |= ((1<<1));
	value &= ~((1<<3)|(1<<0));
	tps65910_reg_write(pmic, TPS65910_DEVCTRL, value);
