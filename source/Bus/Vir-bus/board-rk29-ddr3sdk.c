...
#define MEM_EBC_SIZE       SZ_1M // (SZ_1M+(SZ_2M))
...
#define MEM_EBC_BASE       (MEM_FBIPP_BASE-MEM_EBC_SIZE)
#define PMEM_SKYPE_BASE     (MEM_EBC_BASE - PMEM_SKYPE_SIZE)
...
#if defined (CONFIG_TOUCHSCREEN_GT819) || (CONFIG_TOUCHSCREEN_GT813)
#define TOUCH_RESET_PIN RK29_PIN6_PC3
#define TOUCH_INT_PIN   RK29_PIN0_PA2
...
static struct rk29_adc_battery_platform_data rk29_adc_battery_platdata = {
	.dc_det_pin      = RK29_PIN0_PA0,
	.batt_low_pin    = RK29_PIN4_PA2,
	.chglev  = RK29_PIN5_PD7,
	.acin = RK29_PIN6_PD0,
	.charge_ok_pin   = RK29_PIN4_PA3,

	.dc_det_level    = GPIO_LOW,
	.charge_ok_level = GPIO_HIGH,
	.chglev_level = GPIO_HIGH,
	.acin_level = GPIO_HIGH,
};
...
/*****************************************************************************************
 * EBC devices
 * author: hl@rock-chips.com
*****************************************************************************************/
/* rk29 ebc resource */
#ifdef CONFIG_FB_RK29_EBC
#if CONFIG_MACH_RK29_2906
#define EBC_POWER_ON_PIN RK29_PIN4_PD0//RK29_PIN3_PD0
#define EBC_POWER_ON_IOMUX_NAME NULL//GPIO3D0_SMCADDR18_HOSTADDR0_NAME
#define EBC_POWER_ON_IOMUX_VALUE 0
#define EBC_POWER_ON_VB_PIN RK29_PIN6_PD1

#define EBC_VCOM_PIN			RK29_PIN6_PC1

extern int lcdc_disable_and_put_clk(void);
#else
#define EBC_POWER_ON_PIN RK29_PIN3_PD0
#define EBC_POWER_ON_IOMUX_NAME GPIO3D0_SMCADDR18_HOSTADDR0_NAME
#define EBC_POWER_ON_IOMUX_VALUE 0
#endif
...
{
	int reg;
	reg = readl(RK29_GRF_BASE + 0xc0);
	writel((reg | 3 << 22), (RK29_GRF_BASE + 0xc0));
	reg = readl(RK29_GRF_BASE + 0xc0);
}

        gpio_direction_output(EBC_POWER_ON_PIN,GPIO_LOW);
        gpio_direction_output(EBC_POWER_ON_VB_PIN,GPIO_HIGH);
#else
        gpio_direction_output(EBC_POWER_ON_PIN, GPIO_HIGH);
#endif

	return 0;
}
static int ebc_power_off(void)
{
#if CONFIG_MACH_RK29_2906
{
	int reg;
	...
	reg = readl(RK29_GRF_BASE + 0xc0);
	writel((reg & 0xFF3FFFFF), (RK29_GRF_BASE + 0xc0));
	reg = readl(RK29_GRF_BASE + 0xc0);

	if(0 == first_in)
	{
		first_in = 1;
		lcdc_disable_and_put_clk();
	}
}

        gpio_direction_output(EBC_POWER_ON_PIN,GPIO_HIGH);
        gpio_direction_output(EBC_POWER_ON_VB_PIN,GPIO_LOW);
#else
        gpio_direction_output(EBC_POWER_ON_PIN, GPIO_LOW);
#endif
...
static  int  ebc_vcom_power_on(void)
{
	#if CONFIG_MACH_RK29_2906
	gpio_direction_output(EBC_VCOM_PIN,GPIO_HIGH);
	#endif
	return 0;
}

static  int  ebc_vcom_power_off(void)
{
	#if CONFIG_MACH_RK29_2906
	gpio_direction_output(EBC_VCOM_PIN,GPIO_LOW);
	#endif
	return 0;
}


static  int ebc_io_init(void)
{
#if defined(CONFIG_EBC_USE_GPIO)
	rk29_mux_api_set(GPIO0B7_EBCGDOE_SMCOEN_NAME, GPIO0L_EBC_GDOE);
	rk29_mux_api_set(GPIO0B6_EBCSDSHR_SMCBLSN1_HOSTINT_NAME, GPIO0L_EBC_SDSHR);
	rk29_mux_api_set(GPIO0B5_EBCVCOM_SMCBLSN0_NAME, GPIO0L_EBC_VCOM);
	rk29_mux_api_set(GPIO0B4_EBCBORDER1_SMCWEN_NAME, GPIO0L_EBC_BORDER1);
	rk29_mux_api_set(GPIO0B3_EBCBORDER0_SMCADDR3_HOSTDATA3_NAME, GPIO0L_EBC_BORDER0);
	rk29_mux_api_set(GPIO0B2_EBCSDCE2_SMCADDR2_HOSTDATA2_NAME, GPIO0L_EBC_SDCE2);
	rk29_mux_api_set(GPIO0B1_EBCSDCE1_SMCADDR1_HOSTDATA1_NAME, GPIO0L_EBC_SDCE1);
	rk29_mux_api_set(GPIO0B0_EBCSDCE0_SMCADDR0_HOSTDATA0_NAME, GPIO0L_EBC_SDCE0);
	rk29_mux_api_set(GPIO0D1_EBCGDCLK_SMCADDR4_HOSTDATA4_NAME, GPIO0H_EBC_GDCLK);
	rk29_mux_api_set(GPIO0D0_EBCSDOE_SMCADVN_NAME, GPIO0H_EBC_SDOE);
	rk29_mux_api_set(GPIO0C7_EBCSDCE5_SMCDATA15_NAME, GPIO0H_EBC_SDCE5);
	rk29_mux_api_set(GPIO0C6_EBCSDCE4_SMCDATA14_NAME, GPIO0H_EBC_SDCE4);
	rk29_mux_api_set(GPIO0C5_EBCSDCE3_SMCDATA13_NAME, GPIO0H_EBC_SDCE3);
	rk29_mux_api_set(GPIO0C4_EBCSDCE2_SMCDATA12_NAME, GPIO0H_EBC_GDPWR2);
	rk29_mux_api_set(GPIO0C3_EBCSDCE1_SMCDATA11_NAME, GPIO0H_EBC_GDPWR1);	
	rk29_mux_api_set(GPIO0C2_EBCSDCE0_SMCDATA10_NAME, GPIO0H_EBC_GDPWR0);	
	rk29_mux_api_set(GPIO0C1_EBCGDR1_SMCDATA9_NAME, GPIO0H_EBC_GDRL);	
	rk29_mux_api_set(GPIO0C0_EBCGDSP_SMCDATA8_NAME, GPIO0H_EBC_GDSP);
	rk29_mux_api_set(GPIO5D1_EBCSDCLK_SMCADDR6_HOSTDATA6_NAME, GPIO5H_EBC_SDCLK);
	rk29_mux_api_set(GPIO5D0_EBCSDLE_SMCADDR5_HOSTDATA5_NAME, GPIO5H_EBC_SDLE);
	rk29_mux_api_set(GPIO5C7_EBCSDDO7_SMCDATA7_NAME, GPIO5H_EBC_SDDO7);
	rk29_mux_api_set(GPIO5C6_EBCSDDO6_SMCDATA6_NAME, GPIO5H_EBC_SDDO6);
	rk29_mux_api_set(GPIO5C5_EBCSDDO5_SMCDATA5_NAME, GPIO5H_EBC_SDDO5);
	rk29_mux_api_set(GPIO5C4_EBCSDDO4_SMCDATA4_NAME, GPIO5H_EBC_SDDO4);
	rk29_mux_api_set(GPIO5C3_EBCSDDO3_SMCDATA3_NAME, GPIO5H_EBC_SDDO3);
	rk29_mux_api_set(GPIO5C2_EBCSDDO2_SMCDATA2_NAME, GPIO5H_EBC_SDDO2);
	rk29_mux_api_set(GPIO5C1_EBCSDDO1_SMCDATA1_NAME, GPIO5H_EBC_SDDO1);
	rk29_mux_api_set(GPIO5C0_EBCSDDO0_SMCDATA0_NAME, GPIO5H_EBC_SDDO0);

	rk29_mux_api_set(EBC_POWER_ON_IOMUX_NAME,EBC_POWER_ON_IOMUX_VALUE);//
#else
	int reg;
	int ret;

	ret = gpio_request(EBC_POWER_ON_PIN, "EBC_POWER_ON_PIN");
	if (ret < 0) 
	{
		printk("Failed to request GPIO:%d, ERRNO:%d", (s32)EBC_POWER_ON_PIN, ret);
		return ret;
	}

	ret = gpio_request(EBC_POWER_ON_VB_PIN, "EBC_POWER_ON_VB_PIN");
	if (ret < 0) 
	{
		printk("Failed to request GPIO:%d, ERRNO:%d", (s32)EBC_POWER_ON_VB_PIN, ret);
		return ret;
	}

	ret = gpio_request(EBC_VCOM_PIN, "EBC_VCOM_PIN");
	if (ret < 0) 
	{
		printk("Failed to request GPIO:%d, ERRNO:%d", (s32)EBC_VCOM_PIN, ret);
		return ret;
	}
...
static struct resource rk29_ebc_resource[] = {
	[0] = {
		.name  = "ebc reg",
		.start = RK29_EBC_PHYS,
		.end   = RK29_EBC_PHYS + RK29_EBC_SIZE - 1,
		.flags = IORESOURCE_MEM,
	},
	[1] = {
		.name  = "ebc irq",
	...
	[2] = {
		.name   = "ebc buf",
		.start  = MEM_EBC_BASE,
		.end    = MEM_EBC_BASE + MEM_EBC_SIZE-1,
		.flags  = IORESOURCE_MEM,
	},
};
...
struct platform_device rk29_device_ebc = {
	.name		  = "rk29-ebc",
	.id		  = 0,
	.num_resources	  = ARRAY_SIZE(rk29_ebc_resource),
	.resource	  = rk29_ebc_resource,
	.dev            = {
...
static struct gpio_switch_platform_data hp_switch_pdata = {
	.name	= "h2w",
	.gpio	= RK29_PIN1_PD7,
};

struct platform_device rk29_hp_switch = {
	.name	= "switch-gpio",
	.dev	= {
		.platform_data = &hp_switch_pdata,
	},
};
#endif
...
#define MMA7660_INT_PIN   RK29_PIN6_PD2
...
static int mma7660_init_platform_hw(void)
{
    ...
    gpio_pull_updown(MMA7660_INT_PIN, PullEnable);
    return 0; 
}
...
/*************************************PMU ACT8931****************************************/
		...
		/*dcdc mode*/
/*act8931 in REGULATOR_MODE_STANDBY mode is said DCDC is in PMF mode is can save power,when in REGULATOR_MODE_NORMAL 
mode is said DCDC is in PWM mode , General default is in REGULATOR_MODE_STANDBY mode*/
		/*ldo mode */
/*act8931 in REGULATOR_MODE_STANDBY mode is said LDO is in low power mode is can save power,when in REGULATOR_MODE_NORMAL 
mode is said DCDC is in nomal mode , General default is in REGULATOR_MODE_STANDBY mode*/
/*set dcdc and ldo voltage by regulator_set_voltage()*/
int act8931_set_init(struct act8931 *act8931)
{
	...
	act_ldo1 = regulator_get(NULL, "act_ldo1");
	regulator_enable(act_ldo1); 
	regulator_set_voltage(act_ldo1,2500000,2500000);
	tmp = regulator_get_voltage(act_ldo1);
	regulator_set_mode(act_ldo1,REGULATOR_MODE_STANDBY);
	//regulator_set_mode(act_ldo1,REGULATOR_MODE_NORMAL);
	printk("***regulator_set_init: ldo1 vcc =%d\n",tmp);
	...
	act_ldo2 = regulator_get(NULL, "act_ldo2");
	regulator_enable(act_ldo2);
	regulator_set_voltage(act_ldo2,1800000,1800000);
	tmp = regulator_get_voltage(act_ldo2);
	regulator_set_mode(act_ldo2,REGULATOR_MODE_STANDBY);
	//regulator_set_mode(act_ldo2,REGULATOR_MODE_NORMAL);
	printk("***regulator_set_init: ldo2 vcc =%d\n",tmp);
	...
	act_ldo3 = regulator_get(NULL, "act_ldo3");
	regulator_enable(act_ldo3);
	regulator_set_voltage(act_ldo3,3000000,3000000);
	tmp = regulator_get_voltage(act_ldo3);
	regulator_set_mode(act_ldo3,REGULATOR_MODE_STANDBY);
	//regulator_set_mode(act_ldo3,REGULATOR_MODE_NORMAL);
	printk("***regulator_set_init: ldo3 vcc =%d\n",tmp);
	...
	act_ldo4 = regulator_get(NULL, "act_ldo4");
	regulator_enable(act_ldo4);
	regulator_set_voltage(act_ldo4,3000000,3000000);
	tmp = regulator_get_voltage(act_ldo4);
	regulator_set_mode(act_ldo4,REGULATOR_MODE_STANDBY);
	//regulator_set_mode(act_ldo4,REGULATOR_MODE_NORMAL);
	printk("***regulator_set_init: ldo4 vcc =%d\n",tmp);
	...
	/*init dcdc3*/
	act_dcdc3 = regulator_get(NULL, "vcore");
	regulator_enable(act_dcdc3);
        printk("cc_test_1\n");
	regulator_set_voltage(act_dcdc3,1250000, 1250000);
        printk("cc_test_2\n");
	tmp = regulator_get_voltage(act_dcdc3);
	regulator_set_mode(act_dcdc3,REGULATOR_MODE_STANDBY);
	//regulator_set_mode(act_dcdc3,REGULATOR_MODE_NORMAL);
...
static struct regulator_consumer_supply act8931_ldo1_consumers[] = {
	{
		.supply = "act_ldo1",
	}
};

static struct regulator_init_data act8931_ldo1_data = {
	.constraints = {
		.name = "ACT_LDO1",
		.min_uV = 600000,
		.max_uV = 3900000,
	...
	.num_consumer_supplies = ARRAY_SIZE(act8931_ldo1_consumers),
	.consumer_supplies = act8931_ldo1_consumers,
};

/**/
static struct regulator_consumer_supply act8931_ldo2_consumers[] = {
	{
		.supply = "act_ldo2",
	}
};

static struct regulator_init_data act8931_ldo2_data = {
	.constraints = {
		.name = "ACT_LDO2",
		.min_uV = 600000,
		.max_uV = 3900000,
	...
	.num_consumer_supplies = ARRAY_SIZE(act8931_ldo2_consumers),
	.consumer_supplies = act8931_ldo2_consumers,
};

/*ldo3 VCC_NAND WIFI/BT/FM_BCM4325*/
static struct regulator_consumer_supply act8931_ldo3_consumers[] = {
	{
		.supply = "act_ldo3",
	}
};

static struct regulator_init_data act8931_ldo3_data = {
	.constraints = {
		.name = "ACT_LDO3",
		.min_uV = 600000,
		.max_uV = 3900000,
	...
	.num_consumer_supplies = ARRAY_SIZE(act8931_ldo3_consumers),
	.consumer_supplies = act8931_ldo3_consumers,
};

/*ldo4 VCCA CODEC_WM8994*/
static struct regulator_consumer_supply act8931_ldo4_consumers[] = {
	{
		.supply = "act_ldo4",
	}
};

static struct regulator_init_data act8931_ldo4_data = {
	.constraints = {
		.name = "ACT_LDO4",
		.min_uV = 600000,
		.max_uV = 3900000,
	...
	.num_consumer_supplies = ARRAY_SIZE(act8931_ldo4_consumers),
	.consumer_supplies = act8931_ldo4_consumers,
};
/*buck1 vcc Core*/
static struct regulator_consumer_supply act8931_dcdc1_consumers[] = {
	{
		.supply = "act_dcdc1",
	}
};

static struct regulator_init_data act8931_dcdc1_data = {
	.constraints = {
		.name = "ACT_DCDC1",
		.min_uV = 600000,
		.max_uV = 3900000,
	...
	.num_consumer_supplies = ARRAY_SIZE(act8931_dcdc1_consumers),
	.consumer_supplies = act8931_dcdc1_consumers
};

/*buck2 VDDDR MobileDDR VCC*/
static struct regulator_consumer_supply act8931_dcdc2_consumers[] = {
	{
		.supply = "act_dcdc2",
	}
};

static struct regulator_init_data act8931_dcdc2_data = {
	.constraints = {
		.name = "ACT_DCDC2",
		.min_uV = 600000,
		.max_uV = 3900000,
	...
	.num_consumer_supplies = ARRAY_SIZE(act8931_dcdc2_consumers),
	.consumer_supplies = act8931_dcdc2_consumers
};

/*buck3 vdd Core*/
static struct regulator_consumer_supply act8931_dcdc3_consumers[] = {
	{
		.supply = "vcore",
	}
};

static struct regulator_init_data act8931_dcdc3_data = {
	.constraints = {
		.name = "ACT_DCDC3",
		.min_uV = 600000,
		.max_uV = 3900000,
	...
	.num_consumer_supplies = ARRAY_SIZE(act8931_dcdc3_consumers),
	.consumer_supplies = act8931_dcdc3_consumers
};

struct act8931_regulator_subdev act8931_regulator_subdev_id[] = {
	{
		.id=0,
		.initdata=&act8931_ldo1_data,		
	 },

	{
		.id=1,
		.initdata=&act8931_ldo2_data,		
	 },

	{
		.id=2,
		.initdata=&act8931_ldo3_data,		
	 },

	{
		.id=3,
		.initdata=&act8931_ldo4_data,		
	 },

	{
		.id=4,
		.initdata=&act8931_dcdc1_data,		
	 },

	{
		.id=5,
		.initdata=&act8931_dcdc2_data,		
	 },
	{
		.id=6,
		.initdata=&act8931_dcdc3_data,		
	 },
...
#ifdef	CONFIG_GS_SENSOR
#define SENSOR_INT_PIN	RK29_PIN6_PD2
static int sensor_init_platform_hw(void) {
	int ret = gpio_request(SENSOR_INT_PIN,NULL);
	if(ret != 0) {
		gpio_free(SENSOR_INT_PIN);
		printk("%s gpio_request error\n", __FUNCTION__);
		return -EIO;
	}
	gpio_pull_updown(SENSOR_INT_PIN, PullEnable);
	gpio_free(SENSOR_INT_PIN);
	return 0;
}

struct rk29_gsensor_platform_data gsensor_data = {
		.init_platform_hw = sensor_init_platform_hw,
};
#endif
#ifdef CONFIG_RK29_I2C1_CONTROLLER
...
#if defined(CONFIG_PMIC_AXP192) || defined(CONFIG_PMIC_AXP192_MODULE)
	{
		I2C_BOARD_INFO("axp192", 0x34),
		.platform_data = &axp192_platform_data,
	},
#endif
#if defined (CONFIG_BATTERY_BQ27541)
	{
...
#define GPU_HIGH_CLOCK      144//  552
#define GPU_LOW_CLOCK        144// (periph_pll_default / 1000000) /* same as general pll clock rate below */
...
const static unsigned int palmtc_keypad_row_gpios[] = {
	RK29_PIN4_PA1, RK29_PIN4_PA2, RK29_PIN4_PA3
};

const static unsigned int palmtc_keypad_col_gpios[] = {
	RK29_PIN1_PB7, RK29_PIN1_PB6, RK29_PIN2_PB1, RK29_PIN2_PB0, RK29_PIN2_PA7 };
...
static struct platform_device rk29_matrix_keys = {
	.name		= "matrix-keypad",
	.id		= -1,
	.dev	= {
...
static struct platform_device *devices[] __initdata = {
	...
	&rk29_device_backlight,
#endif
#if defined(CONFIG_BACKLIGHT_BOEYE)
	&rk29_device_boeye_backlight,
#endif
...
#endif
#if defined(CONFIG_WAVEFORM_FROM_EEPROM)
    {   /*spi flash add by dlx 2012-05-21*/
        .modalias   = "epd_spi_flash",
        .chip_select    = 0,
        .max_speed_hz   = 10*1000000,//20*1000000,
        .bus_num    = 0,        
    },
#endif
};
...
static struct cpufreq_frequency_table freq_table[] = {
	{ .index = 1050000, .frequency =  102000 },
...
static void __init switch_hp_gpio_init(void)
{
	rk29_mux_api_set(GPIO1D7_SDMMC0DATA5_NAME, GPIO1H_GPIO1D7);
}
...
#if CONFIG_RTL8188EUS
	// wifi vcc con io.
	/*
	if (gpio_request(RK29_PIN6_PB2, "GPIO_WIFI_POWER") < 0) {
		pr_err("%s: gpio_request(%d) failed\n", __func__, RK29_PIN6_PB2);
		return ;
	}
	gpio_direction_output(RK29_PIN6_PB2, GPIO_HIGH);
	gpio_free(RK29_PIN6_PB2);
	*/
#endif
...
#define ReadReg32(addr)                     (*(int *)(addr))
#define WriteReg32(addr, data)              (*(int *)(addr) = data)
static void __init machine_rk29_mapio(void)
{
	rk29_map_common_io();
	rk29_setup_early_printk();
	rk29_sram_init();
	rk29_clock_init2(periph_pll_default, codec_pll_300mhz, false);
	rk29_iomux_init();
	ddr_init(DDR_TYPE, DDR_FREQ);
	
	/*AXI priority*/
	WriteReg32((RK29_GRF_BASE+0xac), (ReadReg32((RK29_GRF_BASE+0xac))&0xFFFFFC00) | VCODEC_PRIO(2) | GPU_PRIO(2) | LCD_PRIO(0) | PERI_PRIO(1) | CPU_PRIO(2));
